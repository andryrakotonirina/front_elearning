...
export default function CardChapitre(onClick) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('http://localhost:9090/api/achats/chapitres');
  ...
  return (
    <MDBRow className='row-cols-1 row-cols-md-3 g-4'  >{chapitre.map((item) => {
      return (
      <MDBCol  key={item.id}> 
          ...
          <MDBCardImage
            ...
            alt="Photo du cours qui montre le Video du chaptitre"
            position='top'
          />
          <MDBCardBody>
            <MDBCardTitle>Le titre de ce chapitre est {item.Titre}</MDBCardTitle>
            <MDBCardText>
            <span> {item.Sequence}Ar,</span>
            <span>{item.ResumeChapitre}</span>
          ...
          <MDBCardFooter className='justify-content-center'>
            ...
            <MDBBtn className='me-1'onClick={()=> navigate()}>
